...
with tab2:
    ...
    if uploaded:
        ...
        best=ir.iloc[0]
        st.markdown(f'<div class="warning"><b>Analysis principle.</b> The image is analysed as a distribution of CMYK colour values. Representative CMYK values are evaluated independently by the AI-PQI model. Each predicted DeltaE00 is then weighted by the proportion of pixels represented by that colour. The <b>primary recommendation criterion is the pixel-weighted mean predicted DeltaE00 across the image</b>; the dominant colour alone is not used to select the paper or screening. P90 DeltaE00 is reported as a secondary indicator of the higher-error part of the colour distribution. Current analysis uses {len(colors)} representative CMYK colours; the most frequent colour represents {float(np.max(weights))*100:.1f}% of analysed pixels.</div>',unsafe_allow_html=True)
        st.markdown('<div style="height:1rem"></div>',unsafe_allow_html=True)
